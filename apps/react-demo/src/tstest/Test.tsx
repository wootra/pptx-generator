...
export function testMainMethods() {
	...
	pptx.writeFile({ fileName: 'testFile' }).then((fileName: string) =>
...
function basicDemoSlide(pptx: PptxGenJS) {
	// LEGACY-TEST: @deprecated in v3.3.0
	//pptx.addSlide("masterName"); // slide0

	pptx.addSection({ title: 'TypeScript' } as SectionProps);
...
function testMethod_Chart(pptx: PptxGenJS) {
	let slide = pptx.addSlide();
...
function testMethod_Table(pptx: PptxGenJS) {
	...
	// SLIDE 1: Table text alignment and cell styles
	{
		...
		let arrTabRows1: TableRow[] = [
			...
			[
				...
				{
					text: 'Mdl Ctr',
					options: { valign: 'middle', align: 'center' },
				} as TableCell,
				{
					text: 'Mdl Rgt',
					options: { valign: 'middle', align: 'right' },
				} as TableCell,
			],
			[
				{
					text: 'Btm Lft',
					options: { valign: 'bottom', align: 'left' },
				} as TableCell,
				{
					text: 'Btm Ctr',
					options: { valign: 'bottom', align: 'center' },
				} as TableCell,
				{
					text: 'Btm Rgt',
					options: { valign: 'bottom', align: 'right' },
				} as TableCell,
			],
		];
		...
		let arrTabRows2: TableRow[] = [
			...
			[
				...
				{
					text: '10pt Pad',
					options: { fill: { color: '6699CC' }, margin: 10 },
				},
			],
		];
		...
		slide.addTable(arrTabRows33 as TableRow[], {
			x: 0.5,
			y: 4.0,
			rowH: [1, 2],
			colW: [1, 2, 2.5, 3, 4],
	...
	// SLIDE 2: Table row/col-spans
	{
		...
		var arrTabRows3: TableRow[] = [
			[
				{
					text: 'A1\nA2\nA3',
					options: { rowspan: 3, fill: { color: 'FFFCCC' } },
				},
				{
					text: 'B1\nB2',
					options: { rowspan: 2, fill: { color: 'FFFCCC' } },
				},
				{ text: 'C1' },
			],
			[{ text: 'C2' }],
	...
	// SLIDE 3:
	{
		...
		let arrTextObjects: TableRow[] = [
			[
				...
				{
					text: '2nd cell',
					options: { color: '0000EE', align: 'center' },
				},
				{
					text: 'Hyperlink',
					options: {
...
*/
function testMethod_Media(pptx: PptxGenJS) {
	let slide = pptx.addSlide();
...
function testMethod_Shape(pptx: PptxGenJS) {
	let slide = pptx.addSlide();
...
function testMethod_Text(pptx: PptxGenJS) {
	let slide = pptx.addSlide();
...
function testMethod_Masters(pptx: PptxGenJS) {
	const starlabsBkgd = {
		...
		objects: [
			{
				text: {
					text: '(4.0 inches H)',
					options: { ...TEXT_PROPS, ...{ x: 0, y: 0, w: 1, h: 4.0 } },
				},
			},
			{
				text: {
					text: '(1.5 inches H)',
					options: { ...TEXT_PROPS, ...{ x: 1, y: 0, w: 1, h: 1.5 } },
				},
			},
			{
				text: {
...
export function testTableMethod() {
	...
	// PPTX Export Method 1:
	pptx.writeFile({ fileName: 'html2ppt' }).then((fileName: string) =>
		console.log(`writeFile: ${fileName}`)
	);
}
