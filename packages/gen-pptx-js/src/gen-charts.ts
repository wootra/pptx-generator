	...
	return await new Promise((resolve, reject) => {
				...
				if (!IS_MULTI_CAT_AXES) {
					...
					if (Array.isArray(data[0].labels[0])) {
						data[0].labels[0].forEach((_cat, idx) => {
							...
							for (
								let idx2 = data[0].labels.length - 1;
								idx2 >= 0;
								idx2--
							) {
						...
						console.error(
							'wrong data format. it should be multicategory'
						);
					}
				} else {
					// A: create header row
					...
					for (let idx = 0; idx < TOT_CAT; idx++) {
						...
						revLabelGroups.forEach((labelsGroup, idy) => {
							...
							const prev = revLabelGroups[idy - 1];
							if (
								colLabel &&
								prev &&
								prev !== 'string' &&
								...
								const totGrpLbls =
									idy === 0
										...
										: prev.filter(
												label => label && label !== ''
											).length; // get unique label so we can add to get proper shared-string #
		...
		zipExcel
			...
			.then(content => {
				...
				zip.file(
					`ppt/embeddings/Microsoft_Excel_Worksheet${chartObject.globalId}.xlsx`,
					content,
					{ base64: true }
				);
...
export function makeXmlCharts(rel: ISlideRelChart): string {
	...
	if (Array.isArray(rel.opts._type)) {
		rel.opts._type.forEach(type => {
			...
			const valAxisId = options.secondaryValAxis
				? AXIS_ID_VALUE_SECONDARY
				: AXIS_ID_VALUE_PRIMARY;
	...
	if (
		...
		if (rel.opts.valAxes) {
			strXml += makeValAxis(
				{ ...rel.opts, ...rel.opts.valAxes[0] },
				AXIS_ID_VALUE_PRIMARY
			);
			if (rel.opts.valAxes[1]) {
				strXml += makeValAxis(
					{ ...rel.opts, ...rel.opts.valAxes[1] },
					AXIS_ID_VALUE_SECONDARY
				);
			}
		} else {
...
function makeCatAxis(
	...
	strXml += !opts.catAxisLineShow
		...
		: '<a:solidFill>' +
			createColorElement(
				opts.catAxisLineColor || DEF_CHART_GRIDLINE.color
			) +
			'</a:solidFill>';
	strXml +=
...
function makeValAxis(opts: IChartOptsLib, valAxisId: string): string {
	...
	strXml += !opts.valAxisLineShow
		...
		: '<a:solidFill>' +
			createColorElement(
				opts.valAxisLineColor || DEF_CHART_GRIDLINE.color
			) +
			'</a:solidFill>';
	strXml +=
 ...
 */
function genXmlTitle(
	opts: IChartPropsTitle,
	chartX?: number,
	chartY?: number
): string {
	const align =
