...
import {
	execGenSlidesFuncs,
	runEveryTest,
} from '@repo/demo-modules/modules/demos';
import {
	TABLE_NAMES_F,
	TABLE_NAMES_L,
	LOREM_IPSUM,
} from '@repo/demo-modules/modules/enums';
import {
	BKGD_STARLABS,
	CHECKMARK_GRN,
	LOGO_STARLABS,
...
export function doAppStart() {
	...
	// STEP 2: Introduction tab: Library Info
	{
		if (typeof Promise !== 'function') {
			...
			$('#infoLbl_PptxVers').prepend(
				`<span class="cursor-help me-1" title="${pptx.version}">${SVG_INFO_CIRCLE}</span>`
			);
	...
	for (let idx = 0; idx < 36; idx++) {
		$('#tabLotsOfLines tbody').append(
			'<tr><td>Row-' + idx + '</td><td>Col-B</td><td>Col-C</td></tr>'
		);
	}

	// LAST: Re-highlight code
	$('.tab-content code.language-javascript').each(function (_idx, ele) {
...
export function buildDataTable() {
	...
	const rowsVal = $('#numTab2SlideRows').val();
	const rows: number = Array.isArray(rowsVal)
		? parseInt(rowsVal[0])
		: (rowsVal as number);
	// STEP 2:
	for (let idx = 0; idx < rows; idx++) {
		const strHtml =
			...
			LOREM_IPSUM.substring(
				0,
				(Math.floor(Math.random() * 10) + 2) * 130
			) +
			'</td>' +
			'</tr>';
	...
	$('#tabAutoPaging tbody tr:first-child td:last-child').css(
		'font-size',
		'12px'
	);
	$('#tabAutoPaging tbody tr:last-child td:last-child').css(
		'font-size',
		'16px'
	);
}

export function table2slidesDemoForTab(inTabId, inOpts?: any) {
	const pptx = new PptxGenJS();
...
export function table2slides1() {
	...
	if ($('#slideStartY').val())
		objOpts.autoPageSlideStartY = Number($('#slideStartY').val());
	if ($('#selSlideMaster').val())
		objOpts.masterSlideName = $('#selSlideMaster').val();
...
export function table2slides2(addImage) {
	...
	if ($('#slideStartY').val())
		objOpts.newSlideStartY = Number($('#slideStartY').val()); // TEST: DEPRECATED: `newSlideStartY`
	if ($('#selSlideMaster').val())
		objOpts.masterSlideName = $('#selSlideMaster').val();
	...
	if (addImage) {
		...
		*/
		objOpts.addImage = {
			image: { data: CHECKMARK_GRN },
			options: { x: 12.67, y: 0.0, w: 0.67, h: 0.67 },
		};
	}
...
function doNavRestore() {
	...
	const tabTarget = window.location.href.substring(
		window.location.href.toLowerCase().indexOf('.html#') + 6
	);
	const triggerEl = document.querySelector(
		`#myTab button[data-bs-target="#tab-${tabTarget}"]`
	);
...
//     slide.addTable([[{ text: 'Simple Example', options: optsTitle }]], {
//         x: 0.5,
//         y: 0.13,
//         w: 12.5,
//     });
...
//     slide.addText('Line-Level Formatting:', {
//         x: 0.5,
//         y: 1.5,
//         w: '40%',
//         h: 0.38,
...
//     slide.addText('Bullets: Numbered', {
//         x: 8.0,
//         y: 3.4,
//         w: '40%',
//         h: 0.38,
...
export function testTTS() {
	...
	*/
	...
	pptx.tableToSlides('tabAutoPaging', {
		verbose: true,
		autoPageRepeatHeader: true /*, autoPageSlideStartY:2*/,
	});
...
export function table2slidesBullets() {
	const pptx = new PptxGenJS();
