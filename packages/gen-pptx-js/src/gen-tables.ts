...
function parseTextToLines(
	...
	const CPL =
		...
		((cell.options?.fontSize ? cell.options.fontSize : DEF_FONT_SIZE) /
			FOCO); // Chars-Per-Line
	...
	inputCells.forEach(cell => {
		...
		if (typeof cell.text === 'string') {
			...
			if (cell.options?.breakLine) {
				if (verbose)
					console.log(
						`inputCells: new line > ${JSON.stringify(newLine)}`
					);
...
export function getSlidesForTableRows(
		...
		if (tableRowSlides.length > 1) {
			...
			if (typeof tableProps.autoPageSlideStartY === 'number') {
				emuSlideTabH =
					(tablePropH || presLayout.height) -
					inch2Emu(
						tableProps.autoPageSlideStartY + arrInchMargins[2]
					);
	...
	if (tableProps.verbose) {
		...
		console.log(
			`| tableProps.colW ................................. = ${String(tableProps.colW)}`
		);
		console.log(
		...
		console.log(
			`| tablePropX ...................................... = ${tablePropX / EMU}`
		);
		console.log(
			`| tablePropY ...................................... = ${tablePropY / EMU}`
		);
		console.log(
			`| tablePropW ...................................... = ${tablePropW / EMU}`
		);
		console.log(
			`| tablePropH ...................................... = ${tablePropH / EMU}`
		);
		console.log(
			`| tableCalcW ...................................... = ${tableCalcW / EMU}`
		);
	}
	...
	// STEP 4: Calculate usable width now that total usable space is known (`emuSlideTabW`)
	{
		emuSlideTabW =
			tableCalcW ||
			inch2Emu(
				(tablePropX ? tablePropX / EMU : arrInchMargins[1]) +
					arrInchMargins[3]
			);
		if (tableProps.verbose)
			console.log(
...
export function genTableToSlides(
	...
	if (!document.getElementById(tabEleId))
		throw new Error(
			'tableToSlides: Table ID "' + tabEleId + '" does not exist!'
		);
	...
	firstRowCells.forEach((cell: HTMLElement) => {
		if (cell.getAttribute('colspan')) {
			...
			for (
				let idxc = 0;
				idxc < Number(cell.getAttribute('colspan'));
				idxc++
			) {
				arrTabColW.push(
					Math.round(
	...
	arrTabColW.forEach((colW, idxW) => {
		...
		if (colSelectorMin)
			intMinWidth = Number(
				colSelectorMin.getAttribute('data-pptx-min-width')
			);
		const intSetWidth = 0;
		...
		if (colSelectorSet)
			intMinWidth = Number(
				colSelectorSet.getAttribute('data-pptx-width')
			);
		arrColW.push(
			intSetWidth ||
		...
		document
			...
			.forEach((row: HTMLTableRowElement) => {
				...
				switch (part) {
					...
					default:
						console.log(
							`table parsing: unexpected table part: ${part}`
						);
						break;
				}
			});
	...
	getSlidesForTableRows(
		...
		if (opts.addShape)
			newSlide.addShape(
				opts.addShape.shapeName,
				opts.addShape.options || {}
			);
		if (opts.addTable)
