					...
					if (fs && rel.path.indexOf('http') !== 0) {
						...
						https.get(rel.path, res => {
							...
							res.on('error', _ex => {
								...
								reject(
									new Error(
										`ERROR! Unable to load image (https.get): ${rel.path}`
									)
								);
							});
						});
						...
						xhr.onerror = _ex => {
							...
							reject(
								new Error(
									`ERROR! Unable to load image (xhr.onerror): ${rel.path}`
								)
							);
						};
...
async function createSvgPngPreview(rel: ISlideRelMedia): Promise<string> {
	return await new Promise((resolve, reject) => {
		...
		image.onerror = _ex => {
			...
			reject(
				new Error(
					`ERROR! Unable to load image (image.onerror): ${rel.path}`
				)
			);
		};
 ...
 */
export function getSizeFromImage(inImgUrl: string): {
	width: number;
	height: number;
} {
