...
export function genSlides_Media(pptx: PptxGenJS) {
	pptx.addSection({ title: 'Media' } as SectionProps);
...
function genSlide01(pptx: PptxGenJS) {
	...
	slide.addText(
		[{ text: 'Type: m4v' }] as TextProps[],
		{
			...BASE_CODE_OPTS,
			...{ x: 0.5, y: 0.6, h: 0.4, w: 3.56 },
			...TITLE_STYLE,
		} as TextPropsOptions
	);
	slide.addMedia({
	...
	slide.addText([{ text: '`cover` image provided' }] as TextProps[], {
		...BASE_CODE_OPTS,
		...{ x: 0.5, y: 3.0, h: 0.4, w: 3.56 },
		...CODE_STYLE,
	});
	...
	slide.addText([{ text: 'no `cover` image provided' }] as TextProps[], {
		...BASE_CODE_OPTS,
		...{ x: 9.3, y: 3.0, h: 0.4, w: 3.56 },
		...CODE_STYLE,
	});
	...
	slide.addText(
		[{ text: 'Type: mp4' }] as TextProps[],
		{
			...BASE_CODE_OPTS,
			...{ x: 0.5, y: 3.85, h: 0.4, w: 3.6 },
			...TITLE_STYLE,
		} as TextPropsOptions
	);
	slide.addMedia({
	...
	slide.addText(
		[{ text: 'Type: avi' }] as TextProps[],
		{
			...BASE_CODE_OPTS,
			...{ x: 4.79, y: 3.85, h: 0.4, w: 3.6 },
			...TITLE_STYLE,
		} as TextPropsOptions
	);
	slide.addMedia({
	...
	slide.addText(
		[{ text: 'Type: mov' }] as TextProps[],
		{
			...BASE_CODE_OPTS,
			...{ x: 9.08, y: 3.85, h: 0.4, w: 3.75 },
			...TITLE_STYLE,
		} as TextPropsOptions
	);
	slide.addMedia({
...
function genSlide02(pptx: PptxGenJS) {
	...
	slide.addText(
		[{ text: 'Type: mp3' }] as TextProps[],
		{
			...BASE_CODE_OPTS,
			...{ x: 0.5, y: 0.6, h: 0.4, w: 3.5 },
			...TITLE_STYLE,
		} as TextPropsOptions
	);
	slide.addMedia({
	...
	slide.addText(
		[{ text: 'Type: aiff' }] as TextProps[],
		{
			...BASE_CODE_OPTS,
			...{ x: 4.92, y: 0.6, h: 3.9, w: 3.5 },
			...TITLE_STYLE,
		} as TextPropsOptions
	);
	slide.addMedia({
	...
	slide.addText(
		[{ text: 'Type: wav' }] as TextProps[],
		{
			...BASE_CODE_OPTS,
			...{ x: 9.33, y: 0.6, h: 0.4, w: 3.5 },
			...TITLE_STYLE,
		} as TextPropsOptions
	);
	slide.addMedia({
...
function genSlide03(pptx: PptxGenJS) {
	...
	slide.addText('Online: YouTube', {
		...{ x: 0.5, y: 0.75, h: 5.6, w: 12.3 },
		...TITLE_STYLE,
	} as TextPropsOptions);
	...
	// FOOTER
	slide.addText(
		'Note: YouTube videos require newer versions of PowerPoint (v16+/M365). Older versions will show content warning messages.',
		{
 ...
 * @param {PptxGenJS} pptx
 */
export function genSlide_Test_LargeMedia(pptx: PptxGenJS) {
	let slide = pptx.addSlide({ sectionTitle: 'Media' });
	slide.addNotes(
		'API Docs: https://gitbrent.github.io/PptxGenJS/docs/api-media.html'
	);
	...
	slide.addText([{ text: IMAGE_PATHS.big_earth_mp4.path }] as TextProps[], {
		x: 0.5,
		y: 0.5,
		w: 12.2,
		h: 1,
