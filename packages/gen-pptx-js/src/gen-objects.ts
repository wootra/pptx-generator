	...
	if (
		...
		props.objects.forEach((object, idx) => {
			...
			if (MASTER_OBJECTS[key] && key === 'chart') {
				...
				addTextDefinition(
					tgt,
					[{ text: object[key].text }] as TextProps[],
					object[key].options,
					false
				);
...
export function addChartDefinition(
	...
	function correctGridLineOptions(glOpts: OptsChartGridLine): void {
		...
		if (
			...
			console.warn(
				'Warning: chart.gridLine.size must be greater than 0.'
			);
	...
	if (options.dataLabelPosition) {
		...
		if (
			...
			if (
				!['b', 'ctr', 'l', 'r', 't'].includes(options.dataLabelPosition)
			) {
				delete options.dataLabelPosition;
			}
		}
		if (options._type === CHART_TYPE.BAR) {
			...
			if (!['clustered'].includes(options.barGrouping || '')) {
				if (
					!['ctr', 'inBase', 'inEnd', 'outEnd'].includes(
						options.dataLabelPosition
					)
				) {
					delete options.dataLabelPosition;
	...
	if (
		!options.dataLabelFormatScatter &&
		options._type === CHART_TYPE.SCATTER
	) {
...
export function addImageDefinition(target: PresSlide, opt: ImageProps): void {
	...
	if (strImgExtn === 'svg') {
		...
		target._relsMedia.push({
			...
			isSvgPng: true,
			svgSize: {
				w: getSmartParseNumber(
					newObject.options.w,
					'X',
					target._presLayout
				),
				h: getSmartParseNumber(
					newObject.options.h,
					'Y',
					target._presLayout
				),
...
export function addTableDefinition(
			...
			row.forEach((cell: number | string | TableCell) => {
				...
				arrSides.forEach(idx => {
					newCell.options.border[idx] = {
						...
						color:
							newCell.options.border[idx]?.color ||
	...
	*/
	...
	if (opt.w && (opt.w as number) < 20) opt.w = inch2Emu(opt.w);
	if (opt.h && (opt.h as number) < 20) opt.h = inch2Emu(opt.h);
		...
		getSlidesForTableRows(arrRows, opt, presLayout, slideLayout).forEach(
			(slide, idx) => {
				...
				if (!getSlide(target._slideNum + idx)) {
					slides.push(
						addSlide({ masterName: slideLayout?._name || null })
					);
				}
				...
				{
					const newSlide: PresSlide = getSlide(
						target._slideNum + idx
					);

					opt.autoPage = false;

					// Create hyperlink rels (IMPORTANT: Wait until table has been shredded across Slides or all rels will end-up on Slide 1!)
					createHyperlinkRels(newSlide, slide.rows as TableCell[][]);
...
export function addTextDefinition(
	target: PresSlide,
	text: TextProps[],
	opts: TextPropsOptions | undefined,
	...
	function cleanOpts(itemOpts: ObjectOptions): TextPropsOptions {
		// STEP 1: Set some options
		{
			...
			if (
				...
				if (placeHold?.options) {
					itemOpts = { ...itemOpts, ...placeHold.options };
				}
			}
			...
			if (
				typeof itemOpts.underline === 'boolean' &&
		...
		// STEP 2: Transform `align`/`valign` to XML values, store in _bodyProp for XML gen
		{
			if ((itemOpts.align || '').toLowerCase().indexOf('c') === 0) {
				...
				(itemOpts.align || '').toLowerCase().indexOf('l') === 0
			) {
				itemOpts._bodyProp.align = TEXT_HALIGN.left;
			} else if (
				(itemOpts.align || '').toLowerCase().indexOf('r') === 0
			) {
				itemOpts._bodyProp.align = TEXT_HALIGN.right;
			} else if (
				(itemOpts.align || '').toLowerCase().indexOf('j') === 0
			) {
			...
			if ((itemOpts.valign || '').toLowerCase().indexOf('b') === 0) {
				...
				(itemOpts.valign || '').toLowerCase().indexOf('m') === 0
			) {
				itemOpts._bodyProp.anchor = TEXT_VALIGN.ctr;
			} else if (
				(itemOpts.valign || '').toLowerCase().indexOf('t') === 0
			) {
	...
	newObject.options = cleanOpts(newObject.options) as ObjectOptions;

	// STEP 2: Create/Clean text options
	newObject.text.forEach(
		item =>
			(item.options = cleanOpts((item.options || {}) as ObjectOptions))
	);
...
export function addBackgroundDefinition(
	props: BackgroundProps | undefined,
