				...
				*/
				...
				arrTabRows.forEach(cells => {
					for (let cIdx = 0; cIdx < cells.length; ) {
						...
						if (colspan && colspan > 1) {
							const vMergeCells = new Array(colspan - 1)
								.fill(undefined)
								.map(_ => {
									return {
				...
				arrTabRows.forEach((cells, rIdx) => {
					...
					cells.forEach((cell, cIdx) => {
						const rowspan =
							cell._rowContinue || cell.options?.rowspan;
						const colspan = cell.options?.colspan;
			...
			case SLIDE_OBJECT_TYPES.placeholder:
				...
				if (slideItemObj.options.line) {
					...
					if (slideItemObj.options.line.color)
						strSlideXml += genXmlColorSelection(
							slideItemObj.options.line
						);
			...
			case SLIDE_OBJECT_TYPES.image:
				...
				if (sizing?.type) {
					...
					const boxH = sizing.h
						...
						: cy;
					const boxX = getSmartParseNumber(
						sizing.x || 0,
						'X',
						slide._presLayout
					);
					const boxY = getSmartParseNumber(
						sizing.y || 0,
						'Y',
						slide._presLayout
					);
	...
	// A: Build paragraphProperties
	{
		...
		if (typeof textObj.options.bullet === 'object') {
			...
			if (textObj.options.bullet.type) {
				...
				if (
					!/^[0-9A-Fa-f]{4}$/.test(
						textObj.options.bullet.characterCode
					)
				) {
					console.warn(
...
export function genXmlTextBody(slideObj: ISlideObject | TableCell): string {
	...
	*/
	...
	arrTextObjects.forEach((textObj, idx) => {
		if (!textObj.options) textObj.options = {};
		...
		if (arrTexts.length > 0 && (textObj.options?.align || opts.align)) {
			...
			if (
				textObj.options?.align !==
				arrTextObjects[idx - 1].options?.align
			) {
	...
	arrLines.forEach(line => {
		...
		line.forEach((textObj, idx) => {
			// A: Set line index
			if (!textObj.options) {
				textObj.options = {} as TextPropsOptions;
			}
			...
			Object.entries(opts)
				...
				.forEach(([key, val]) => {
					...
					if (
						key !== 'bullet' &&
						!textObj.options?.[key as keyof TextPropsOptions]
					) {
						// @ts-ignore
		...
		if (
			...
			if (opts.fontFace) {
				strSlideXml +=
					...
					(opts.fontSize
						? ` sz="${Math.round(opts.fontSize * 100)}"`
						: '') +
					' dirty="0">';
...
export function genXmlPlaceholder(placeholderObj: ISlideObject): string {
	...
	const placeholderType: string =
		placeholderTyp &&
		PLACEHOLDER_TYPES[placeholderTyp as keyof typeof PLACEHOLDER_TYPES]
			? PLACEHOLDER_TYPES[
					placeholderTyp as keyof typeof PLACEHOLDER_TYPES
				].toString()
			: '';

	return `<p:ph
		${placeholderIdx ? ' idx="' + placeholderIdx.toString() + '"' : ''}
		${placeholderType && PLACEHOLDER_TYPES[placeholderType as keyof typeof PLACEHOLDER_TYPES] ? ` type="${placeholderType}"` : ''}
...
function getLayoutIdxForSlide(
	...
	for (let i = 0; i < slideLayouts.length; i++) {
		if (
			slideLayouts[i]._name ===
			slides[slideNumber - 1]._slideLayout?._name
		) {
			return i + 1;
		}
	}
