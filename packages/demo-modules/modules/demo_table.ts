...
export function genSlides_Table(pptx: PptxGenJS) {
	pptx.addSection({ title: 'Tables' } as SectionProps);
	genSlide01(pptx);
	genSlide02(pptx);
	genSlide03(pptx);
	genSlide04(pptx);
	...
	pptx.addSection({ title: 'Tables: Auto-Paging' } as SectionProps);
	genSlide07(pptx);

	pptx.addSection({ title: 'Tables: Auto-Paging Complex' } as SectionProps);
	genSlide08(pptx);

	pptx.addSection({ title: 'Tables: Auto-Paging Calc' } as SectionProps);
	genSlide09(pptx);

	pptx.addSection({ title: 'Tables: QA' } as SectionProps);
	genSlide10(pptx);
}
 ...
 */
function genSlide01(pptx: PptxGenJS) {
	...
	slide.addTable(
		[
			[
				{
					text: 'Table Examples 1',
					options: BASE_TEXT_OPTS_L as TableProps,
				},
	...
	let arrTabRows2: TableRow[] = [
		[
			{
				text: 'White',
				options: { fill: { color: '6699CC' }, color: 'FFFFFF' },
			},
			{
				text: 'Yellow',
				options: { fill: { color: '99AACC' }, color: 'FFFFAA' },
			},
			{
				text: 'Pink',
				options: { fill: { color: 'AACCFF' }, color: 'E140FE' },
			},
		],
		[
			{
				text: '12pt',
				options: { fill: { color: 'FF0000' }, fontSize: 12 },
			},
			{
				text: '20pt',
				options: { fill: { color: '00FF00' }, fontSize: 20 },
			},
			{
				text: '28pt',
				options: { fill: { color: '0000FF' }, fontSize: 28 },
			},
		],
		[
			{
				text: 'Bold',
				options: { fill: { color: '003366' }, bold: true },
			},
			{
				text: 'Underline',
				options: {
			...
			{
				text: '0.15 margin',
				options: { fill: { color: '6699CC' }, margin: 0.15 },
			},
		],
	];
 ...
 * @param {PptxGenJS} pptx
 */
function genSlide02(pptx: PptxGenJS) {
	let slide = pptx.addSlide({ sectionTitle: 'Tables' });
	...
	slide.addNotes(
		'API Docs: https://gitbrent.github.io/PptxGenJS/docs/api-tables.html'
	);
	...
	let arrTabRows1: TableRow[] = [
		[
			{
				text: 'A1\nA2',
				options: { rowspan: 2, fill: { color: '99FFCC' } },
			},
			{ text: 'B1' },
			{
				text: 'C1 -> D1',
				options: { colspan: 2, fill: { color: '99FFCC' } },
			},
			{ text: 'E1' },
			{
				text: 'F1\nF2\nF3',
				options: { rowspan: 3, fill: { color: '99FFCC' } },
			},
		],
		['B2', 'C2', 'D2', 'E2'],
		['A3', 'B3', 'C3', 'D3', 'E3'],
	...
	let arrTabRows2: TableRow[] = [
		[
			...
			{
				text: 'C1 -> D1',
				options: { colspan: 2, fill: { color: '9999FF' } },
			},
			{
				text: 'E1 -> F1',
				options: { colspan: 2, fill: { color: '9999FF' } },
	...
	let arrTabRows3: TableRow[] = [
		[
			{
				text: 'A1\nA2\nA3',
				options: { rowspan: 3, fill: { color: pptx.colors.ACCENT6 } },
			},
			{
				text: 'B1\nB2',
				options: { rowspan: 2, fill: { color: pptx.colors.ACCENT2 } },
			},
		...
		[
			{
				text: 'B3 -> C3',
				options: { colspan: 2, fill: { color: pptx.colors.ACCENT5 } },
			},
		],
	];
		...
		[
			{
				text: 'A1',
				options: {
					fill: { color: pptx.colors.ACCENT4, transparency: 25 },
				},
			},
			{
				text: 'B1\nB2',
				options: {
					rowspan: 2,
					fill: { color: pptx.colors.ACCENT2, transparency: 25 },
				},
			},
			{
				text: 'C1\nC2\nC3',
				options: {
					rowspan: 3,
					fill: { color: pptx.colors.ACCENT6, transparency: 25 },
				},
			},
		],
		[
		...
		[
			{
				...
				options: {
					colspan: 2,
					fill: { color: pptx.colors.ACCENT5, transparency: 25 },
				},
			},
		],
	];
 ...
 * @param {PptxGenJS} pptx
 */
function genSlide03(pptx: PptxGenJS) {
	let slide = pptx.addSlide({ sectionTitle: 'Tables' });

	slide.addTable(
		[
			[
				{ text: 'Table Examples 3', options: BASE_TEXT_OPTS_L },
				BASE_TEXT_OPTS_R,
			],
		] as TableRow[],
		BASE_TABLE_OPTS
...
function genSlide04(pptx: PptxGenJS) {
	...
	slide.addTable(
		[
			[
				{ text: 'Table Examples 4', options: BASE_TEXT_OPTS_L },
				BASE_TEXT_OPTS_R,
			],
		] as TableRow[],
		BASE_TABLE_OPTS
	...
	slide.addTable(
		[
			[
				{
					text: 'no border and number zero',
					options: { margin: 0.05 },
				},
	...
	let arrTextObjects: TableRow[] = [
		['Plain text', 'Cell 2', 3],
		[
			{
				text: 'Text Objects',
				options: { color: '99ABCC', align: 'right' },
			},
			{ text: '2nd cell', options: { color: '0000EE', align: 'center' } },
			{
				text: 'Hyperlink',
				options: {
					hyperlink: { url: 'https://github.com/gitbrent/pptxgenjs' },
				},
			},
		],
	];
...
function genSlide05(pptx: PptxGenJS) {
	...
	slide.addTable(
		[
			[
				{ text: 'Table Examples 5', options: BASE_TEXT_OPTS_L },
				BASE_TEXT_OPTS_R,
			],
		] as TableRow[],
		BASE_TABLE_OPTS
	...
	let arrTextObjects: TextProps[] = [
		...
		{
			text: '3rd line',
			options: { fontSize: 48, color: '0088CC', align: 'left' },
		},
	];
	slide.addText(arrTextObjects, {
		x: 2.5,
		y: 2.8,
		w: 9.5,
		h: 2,
 ...
 * @param {PptxGenJS} pptx
 */
function genSlide06(pptx: PptxGenJS) {
	let slide = pptx.addSlide({ sectionTitle: 'Tables' });

	slide.addTable(
		[
			[
				{ text: 'Table Examples 6', options: BASE_TEXT_OPTS_L },
				BASE_TEXT_OPTS_R,
			],
		] as TableRow[],
		BASE_TABLE_OPTS
	...
	let arrCell3 = [
		{
			...
			options: {
				...
				hyperlink: { url: 'https://www.google.com' },
			},
		},
	];
	...
	let arrCell4 = [
		{
			text: 'Numbers\nNumbers\nNumbers',
			options: { color: '0088cc', bullet: { type: 'number' } },
		},
	];
	slide.addTable(
		[
			...
			[
				{
					text: [
						{
							text: 'I am a text object with bullets ',
							options: {
								color: 'CC0000',
								bullet: { code: '2605' },
							},
						...
						{
							text: 'and i am the next text object',
							options: {
								color: '00CD00',
								bullet: { code: '25BA' },
							},
						...
						{
							text: 'Final text object w/ bullet:true',
							options: { color: '0000AB', bullet: true },
						},
					],
				},
				{
				...
				},
				{
					text: [
						...
						{
							text: '#',
							options: { fontSize: 60, color: '8648cd' },
						},
						{
							text: '3',
							options: {
...
function genSlide07(pptx: PptxGenJS) {
	let slide: PresSlide | null = null;

	// STEP 1: Build data
	let arrRows = [] as TableRow[];
	let arrText = [] as TableRow[];
	let arrRowsHead1 = [] as TableRow[];
	let arrRowsHead2 = [
		[
			{
				text: 'Title Header',
		...
		TABLE_NAMES_F.forEach((name, idx) => {
			let strText =
				idx == 0
					...
					: LOREM_IPSUM.substring(idx * 100, idx * 200);
	...
	// EX-2: "Paging with smaller table area (50% width)"
	{
		...
		slide.addText(
			[
				{ text: 'Table Examples: ', options: DEMO_TITLE_TEXT },
				{
					text: 'Paging with smaller table area (50% width)',
	...
	{
		slide = pptx.addSlide({
			sectionTitle: 'Tables: Auto-Paging',
			masterName: 'MASTER_AUTO_PAGE_TABLE_PLACEHOLDER',
		});
		slide.addText(
			[
				{ text: 'Table Examples: ', options: DEMO_TITLE_TEXT },
				{
					text: 'Master Page with Auto-Paging',
					options: DEMO_TITLE_OPTS,
				},
			],
			{ x: 0.5, y: 0.13, w: '90%' }
		...
		slide.newAutoPagedSlides.forEach(slide =>
			slide.addText('Auto-Paging table continued...', {
				placeholder: 'footer',
			})
		);
	}
	...
	// EX-4: "Auto-Paging Disabled"
	{
		...
		slide.addTable(arrRows, {
			...
			margin: 0.05,
			border: { color: 'CFCFCF' },
		}); // Negative-Test: no `autoPage:false`
	}
	...
	// EX-5: "Start at `{ y: 4.0 }`, subsequent slides start at slide top margin"
	{
		slide = pptx.addSlide({
			sectionTitle: 'Tables: Auto-Paging',
			masterName: 'MARGIN_SLIDE',
		});
		slide.addText(
			[
				{ text: 'Table Examples: ', options: DEMO_TITLE_TEXT },
				{
					text: 'Start at `{ y:4.0 }`, subsequent slides start at slide top margin',
					options: DEMO_TITLE_OPTS,
				},
			],
	...
	// EX-6: "Start at `{ y: 4.0 }`, subsequent slides start at `{ autoPageSlideStartY: 1.5 }`"
	{
		slide = pptx.addSlide({
			sectionTitle: 'Tables: Auto-Paging',
			masterName: 'MARGIN_SLIDE_STARTY15',
		});
		slide.addText(
			[
				{ text: 'Table Examples: ', options: DEMO_TITLE_TEXT },
	...
	{
		pptx.addSection({
			title: 'Tables: Auto-Paging Repeat Header',
		} as SectionProps);
		slide = pptx.addSlide({
			sectionTitle: 'Tables: Auto-Paging Repeat Header',
		});
		slide.addText(
			[
				{
					text: 'Table Examples: `autoPageHeaderRows`',
					options: DEMO_TITLE_TEXTBK,
				},
				{
					text: 'no `autoPageHeaderRows` prop',
					options: DEMO_TITLE_OPTS,
				},
			],
		...
		slide.addText(
			[
				...
				{
					text: '`{ autoPageHeaderRows: 1 }`',
					options: DEMO_TITLE_OPTS,
				},
			],
			{ x: 4.75, y: 0.13, w: 4, h: 0.4 }
		);
		slide.addTable(arrRowsHead1 as TableRow[], {
			x: 4.75,
			y: 0.6,
			colW: [0.5, 1.0, 2.5],
			margin: 0.05,
	...
	{
		pptx.addSection({
			title: 'Tables: Auto-Paging LineWeight',
		} as SectionProps);
		slide = pptx.addSlide({
			sectionTitle: 'Tables: Auto-Paging LineWeight',
		});
		slide.addText(
			[
				{
					text: 'Table Examples: Line Weight Options',
					options: DEMO_TITLE_TEXTBK,
				},
	...
	{
		pptx.addSection({
			title: 'Tables: Auto-Paging CharWeight',
		} as SectionProps);
		slide = pptx.addSlide({
			sectionTitle: 'Tables: Auto-Paging CharWeight',
		});
		slide.addText(
			[
				{
					text: 'Table Examples: Char Weight Options',
					options: DEMO_TITLE_TEXTBK,
				},
...
function genSlide08(pptx: PptxGenJS) {
	...
	slide.addText(
		[
			{
				text: 'Table Examples: Auto-Paging Using Complex Text Example',
				options: DEMO_TITLE_TEXTBK,
			},
		],
		{
...
function genSlide09(pptx: PptxGenJS) {
	...
	slide.addText(
		[
			{
				text: 'Table Examples: Auto-Paging Calculations',
				options: DEMO_TITLE_TEXTBK,
			},
		],
		{
	...
	for (let rowIdx = 0; rowIdx < 9; rowIdx++) {
		let col3Lines: TextProps[] = [
			{
				text: 'Complex-Title',
				options: { bold: true, color: 'FF0000', breakLine: true },
			},
		];
