 ...
 */
import PptxGenJS, { SectionProps, TableCell } from 'gen-pptx-js';

import { BASE_TABLE_OPTS, BASE_TEXT_OPTS_L, BASE_TEXT_OPTS_R } from './enums';

export function genSlides_Shape(pptx: PptxGenJS) {
	pptx.addSection({ title: 'Shapes' } as SectionProps);
...
function genSlide01(pptx: PptxGenJS) {
	...
	slide.addTable(
		[
			[
				{
					text: 'Shape Examples 1: Misc Shape Types (no text)',
	...
	slide.addNotes(
		'API Docs: https://gitbrent.github.io/PptxGenJS/docs/api-shapes.html'
	);
...
function genSlide02(pptx: PptxGenJS) {
	...
	slide.addTable(
		[
			[
				{
					text: 'Shape Examples 2: Misc Shape Types (with text)',
					options: BASE_TEXT_OPTS_L,
				},
				BASE_TEXT_OPTS_R,
			] as TableCell[],
